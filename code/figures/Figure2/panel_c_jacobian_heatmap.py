# panel_c_jacobian_heatmap.py — Transcriptomics Jacobian heatmap (1:1, no gene labels)
...
df = pd.read_csv(os.path.join(HERE, '..', 'data', 'fig2c_transcriptomics_jacobian_matrix.csv'), index_col='source')
# Full 20x20
...
ax.set_xlabel('Target Gene', fontsize=FS-2, labelpad=4)
ax.set_ylabel('Source Gene', fontsize=FS-2, labelpad=4)

for ext in ['png','pdf','svg']:
    plt.savefig(os.path.join(OUT, f'c.{ext}'), dpi=300)
plt.close(); print('Saved c.png/pdf/svg')
