# panel_j_top_edges.py — Proteomics top edges (lollipop, 1:1)
...
# Abbreviate same-family edges: "pTau-231 - pTau-181" -> "pTau-231-181"
def abbrev(s, t):
    if s == t:
        return s  # self-loop -> single name
    # check common prefix (protein family), e.g. pTau-, BD-pTau-, Aβ
    import re
    # split into family prefix + suffix number
    def split_name(n):
        m = re.match(r'^(.*?)(\d+)$', n)
    ...
    ps, ss = split_name(s)
    pt, st = split_name(t)
    if ps and ps == pt and ss and st:
        return f'{ps}{ss}-{st}'  # e.g. pTau-231-181
    return f'{s}-{t}'  # different families, just dash-join without spaces

df['edge'] = df.apply(lambda r: abbrev(str(r['source']), str(r['target'])), axis=1)
