...
class Step1Config:
    ...
    plasma_h5ad: str = str(config.get_path("paths.processed_data_dir")) + "/plasma_metabolomics_common.h5ad"
    csf_h5ad: str = str(config.get_path("paths.processed_data_dir")) + "/csf_metabolomics_common.h5ad"
    out_dir: str = str(config.get_path("paths.output_dir")) + "/step1_world_model_metabolomics"
    
    # 数据预处理（代谢组：45 配对样本，494 代谢物）
    metabolite_dim: int = None  # 将在加载数据后自动设置
    max_metabolites: int = None  # 不限制（494 代谢物不算多）
...
def train_step1(cfg: Step1Config) -> None:
    ...
    print(f"[DATA] 血浆: {adata_plasma.n_obs} 样本 × {adata_plasma.n_vars} 代谢物")
    print(f"[DATA] CSF: {adata_csf.n_obs} 样本 × {adata_csf.n_vars} 代谢物")
    
    # 找共同代谢物
    common_metabolites = list(set(adata_plasma.var_names) & set(adata_csf.var_names))
    print(f"[DATA] 共同代谢物数: {len(common_metabolites)}")
    
    # 动态设置 metabolite_dim
    if cfg.metabolite_dim is None:
        cfg.metabolite_dim = len(common_metabolites)
        print(f"[DATA] 自动设置 metabolite_dim = {cfg.metabolite_dim}")
    
    # 筛选共同代谢物
    adata_plasma = adata_plasma[:, common_metabolites].copy()
    adata_csf = adata_csf[:, common_metabolites].copy()
    ...
    np.savez_compressed(
        ...
        csf_mean=X_csf_mean,
        csf_std=X_csf_std,
        common_metabolites=np.array(common_metabolites, dtype=object),
    )
