...
OMICS_LIST = ["transcriptomics"]

# ============================================================================
# VGAE 训练参数 — 转录组专用（11,982 基因大网络）
# 与蛋白组（~30 基因）的关键差异：
#   lr: 1e-3 → 1e-4（大网络梯度幅度大，需小学习率避免权重震荡）
#   epochs: 100 → 30（AUROC 在 epoch 15 达峰，后续退化，早停省时间）
#   weight_decay: 1e-4 → 1e-3（加强正则化防止重构退化）
#   out_channels: 2 → 16（大基因空间需要更高维潜空间编码结构）
#   beta: 1.0 → 0.1（降低 KL 约束，让模型更关注图重构质量）
#   n_comp: 3 → 16（PCA 成分匹配潜空间维度）
#   q: 0.5 → 0.85（大网络需严格边过滤，减少噪声连接）
# ============================================================================
OUT_CHANNELS = 16
EPOCHS = 100
LR = 1e-4
...
def aggregate_to_metacells(expr_df, n_metacells=200, min_cells_per_metacell=10):
    """
