#!/usr/bin/env python3
"""
Geneformer 虚拟敲除分析 - 多组学支持（GSE292141 adapter）
...
# 配置路径
PROJECT_ROOT = Path(os.environ.get("PROJECT_ROOT") or Path(__file__).resolve().parents[3])
DATA_PATH = PROJECT_ROOT / "processed-data/step4_single_cell_gse292141/GSE292141_expression_matrix_for_step4.h5ad"
STEP3_OUTPUT_DIR = PROJECT_ROOT / "output/step3_hub_identification"
OUTPUT_BASE_DIR = PROJECT_ROOT / "output/step4_virtual_knockout"
SHARED_CACHE_DIR = OUTPUT_BASE_DIR / "Geneformer_gse292141_shared"
SHARED_TEMP_DIR = SHARED_CACHE_DIR / "temp_data"
SHARED_TOKENIZED_DIR = SHARED_CACHE_DIR / "tokenized_data"

# 组学列表
OMICS_LIST = ["proteomics", "transcriptomics"]  # 运行蛋白组和转录组
...
def load_hub_genes_for_omics(omics_name):
    """从 verified_cross_tissue_edges.csv 动态读取 GenKI 显著边涉及的基因"""
    verified_file = PROJECT_ROOT / "output" / "verified_cross_tissue_edges.csv"
    if not verified_file.exists():
        raise FileNotFoundError(f"验证文件不存在: {verified_file}，请先运行 step4 GenKI 验证")

    df = pd.read_csv(verified_file)
    omics_df = df[df['omics'] == omics_name].copy()

    if omics_df.empty:
        print(f"  WARNING: {omics_name} 在 verified_cross_tissue_edges.csv 中无显著边")
        return [], []

    # Forward (CSF→PBMC): 敲除 source (脑端基因)
    forward_df = omics_df[omics_df['direction'] == 'Forward (CSF→PBMC)']
    brain_genes = sorted(forward_df['source'].unique().tolist())

    # Reverse (PBMC→CSF): 敲除 source (血端基因)
    reverse_df = omics_df[omics_df['direction'] == 'Reverse (PBMC→CSF)']
    blood_genes = sorted(reverse_df['source'].unique().tolist())

    print(f"  从 verified_cross_tissue_edges.csv 读取 {omics_name} 基因")
    print(f"  Forward (KO brain genes): {brain_genes}")
    print(f"  Reverse (KO blood genes): {blood_genes}")
...
def prepare_data(consensus_genes):
    ...
    shared_temp_path = SHARED_TEMP_DIR / "prepared_data.h5ad"

    if shared_temp_path.exists():
        print(f"  复用共享 prepared_data: {shared_temp_path}")
        adata_sample = ad.read_h5ad(shared_temp_path, backed='r')
        ...
        for gene in consensus_genes:
            if gene in adata_sample.var_names and gene in token_dict:
                ...
                print(f"  ⚠️  跳过 {gene}（不在共享 prepared_data 中）")
            else:
                print(f"  ⚠️  跳过 {gene}（不在 token dictionary）")

        adata_sample.file.close()
        if len(valid_genes) == 0:
            raise ValueError("共享 prepared_data 中没有任何有效敲除目标基因")

        print(f"  有效的敲除目标基因: {valid_genes}")
        return shared_temp_path, valid_genes
    ...
    print(f"  同步共享 prepared_data: {shared_temp_path}")
    adata_sample.write_h5ad(shared_temp_path)
...
def tokenize_data(data_path):
    ...
    SHARED_TOKENIZED_DIR.mkdir(parents=True, exist_ok=True)
    
    # 检查是否已经 tokenized
    tokenized_path = tokenized_dir / "tokenized.dataset"
    shared_tokenized_path = SHARED_TOKENIZED_DIR / "tokenized.dataset"
    if shared_tokenized_path.exists():
        print(f"  ⚠️  复用共享 tokenized 数据，跳过")
        return shared_tokenized_path
    if tokenized_path.exists():
        print(f"  ⚠️  已存在本地 tokenized 数据，跳过")
    ...
    tk.tokenize_data(
        ...
        output_directory=str(SHARED_TOKENIZED_DIR),
...
def extract_baseline_embeddings(tokenized_path):
    ...
    isp = InSilicoPerturber(
        ...
        emb_layer=0,
        forward_batch_size=32,  # 提高到32，在当前内存余量下换取速度
        model_version=MODEL_VERSION,
        nproc=1  # WSL多进程问题
    )
...
def select_negative_control_genes(adata, target_gene, n_controls=30, exclude_genes=None):
    """
    选择表达量匹配的阴性对照基因
    """
    import numpy as np
    if target_gene not in adata.var_names:
        return []
    target_expr = adata[:, target_gene].X.mean()
    gene_expr = np.array(adata.X.mean(axis=0)).flatten()
    exclude_set = set(exclude_genes) if exclude_genes else set()
    exclude_set.add(target_gene)
    exclude_mask = np.array([g not in exclude_set for g in adata.var_names])
    expr_diff = np.abs(gene_expr - target_expr)
    expr_diff[~exclude_mask] = np.inf
    top_indices = np.argsort(expr_diff)[:n_controls]
    return [adata.var_names[i] for i in top_indices if expr_diff[i] != np.inf]
...
def post_process_pickles(perturb_dir, valid_genes):
    ...
    阴性对照参照：为每个target基因选表达量匹配的中性基因作为参照锚点，
    计算Cohen's d (信号噪声比 = mean_distance / std_distance)，
    以及所有target基因间的相对排名作为跨基因参照。
    """
    import pickle, os, re
    
    pickle_files = [f for f in os.listdir(perturb_dir) if f.endswith('.pickle') and 'knockout_' in f]
    all_stats = []
    
    # 加载数据用于表达量匹配参照
    adata = None
    data_path = SHARED_TEMP_DIR / "prepared_data.h5ad"
    if not data_path.exists():
        data_path = OUTPUT_DIR / "temp_data" / "prepared_data.h5ad"
    if data_path.exists():
        import scanpy as sc
    ...
    for pickle_file in pickle_files:
        ...
        with open(perturb_dir / pickle_file, "rb") as f:
            data = pickle.load(f)
        ...
        if isinstance(data, dict):
            ...
            if similarities:
                ...
                # 信号噪声比：mean_distance / std_distance
                # 值越大说明敲除效应远超细胞间随机波动
                snr = mean_distance / std_distance if std_distance > 0 else np.nan
                stats['snr'] = snr
                
                # 选表达量匹配的参照基因（不跑敲除，仅作表达量锚点）
                if adata is not None and gene in adata.var_names:
                    cg = select_negative_control_genes(
                        adata, gene, n_controls=10, exclude_genes=set(valid_genes)
                    )
                    cg_in_data = [g for g in cg if g in adata.var_names]
                    target_expr = float(adata[:, gene].X.mean())
                    ctrl_exprs = [float(adata[:, g].X.mean()) for g in cg_in_data[:5]]
                    
                    stats['target_expr_mean'] = target_expr
                    stats['ctrl_expr_mean'] = np.mean(ctrl_exprs) if ctrl_exprs else np.nan
                    stats['ctrl_expr_range'] = f"{min(ctrl_exprs):.4f}-{max(ctrl_exprs):.4f}" if ctrl_exprs else 'N/A'
                    stats['n_ref_genes'] = len(cg_in_data)
                    stats['ref_genes'] = ', '.join(cg_in_data[:8])
                    
                    print(f"      distance={mean_distance:.6f} ± {std_distance:.6f}")
                    print(f"      SNR={snr:.3f} | 表达量: target={target_expr:.4f}, ref={np.mean(ctrl_exprs):.4f}")
                else:
                    print(f"      distance={mean_distance:.6f} ± {std_distance:.6f}, SNR={snr:.3f}")
                
                all_stats.append(stats)
            else:
                print(f"      ⚠️  未找到相似度分数")
        else:
            print(f"      ⚠️  未知pickle格式: {type(data)}")
    
    # 跨基因参照：所有target基因的分布作为互相参照
    if all_stats and len(all_stats) >= 2:
        all_distances = [s['mean_distance'] for s in all_stats]
        median_all = np.median(all_distances)
        for s in all_stats:
            s['cross_gene_ratio'] = s['mean_distance'] / median_all if median_all > 0 else np.nan
            s['cross_gene_rank'] = sorted(all_distances, reverse=True).index(s['mean_distance']) + 1
    ...
    if all_stats:
        ...
        print(f"\n  ✅ 统计结果保存到: {csv_file}")
        top = stats_df.iloc[0]
        print(f"  效应最强: {top['gene']} (距离={top['mean_distance']:.6f}, SNR={top.get('snr', 'N/A'):.4f})")
        if len(stats_df) >= 2:
            print(f"  跨基因参照中位数距离: {median_all:.6f}")
    else:
        print(f"\n  ⚠️  没有生成统计结果")
