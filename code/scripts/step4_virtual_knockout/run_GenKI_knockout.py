...
# 输入输出路径
SC_DATA_PATH = rstr(config.get_path("paths.processed_data_dir")) + "/step4_single_cell_no3/GSE_NO3_expression_matrix.h5ad"
...
def virtual_knockout_genki_cross_tissue(
    ...
    print(f"  Strategy: KO in CSF -> Observe in PBMC")
    print(f"{'=' * 80}")
    ...
    # ========================================================================
    # Step 2: 执行跨组织虚拟敲除（只在 CSF 中敲除）
    # ========================================================================
    print(f"\n[Step 2/6] Performing cross-tissue virtual knockout...")

    # 复制数据（使用 copy 避免修改原始数据）
    ko_csf_data_df = csf_data_df.copy()
    ko_pbmc_data_df = pbmc_data_df  # PBMC 不敲除，不需要复制
