...
if len(common_genes) > N_GENES:
    ...
    def compute_var(adata, gene_indices):
        ...
        for start in range(0, adata.n_obs, chunk_size):
            ...
            chunk = np.asarray(adata.X[start:end, gene_indices], dtype=np.float32)
...
def load_all(adata, indices, gene_idx, mean, std):
    ...
    X = np.asarray(adata.X[idx_sorted, gene_idx], dtype=np.float32)
...
# For correlation, compute pairwise gene means as "mapping"
X_blood_val_mean_gene = X_blood_val.mean(axis=1, keepdims=True)
X_brain_val_mean_gene = X_brain_val.mean(axis=1, keepdims=True)
direct_corr = np.mean([np.corrcoef(X_blood_val[:,i], X_brain_val[:,i])[0,1]
                        for i in range(X_brain_val.shape[1])])
...
if not ckpt_path.exists():
    ...
    t0 = time.time()
    X_blood_val_t = torch.from_numpy(X_blood_val).float()
    t_start = torch.tensor(0.0).float()
    t_end = torch.tensor(1.0).float()

    # Process in batches due to memory
    n_val = len(X_blood_val)
    X_brain_pred_ode = np.zeros_like(X_blood_val)
    batch_size_ode = 200
    ...
    neural_corr = np.mean([np.corrcoef(X_brain_pred_ode[:,i], X_brain_val[:,i])[0,1]
                           for i in range(X_brain_val.shape[1])])
