...
def _kl_mvn(m0, S0, m1, S1):
    ...
    try:
        ...
        # SVD不收敛：添加正则化项后重试
        S1_reg = S1 + np.eye(S1.shape[0]) * 1e-6
        try:
            iS1 = np.linalg.pinv(S1_reg)
        except np.linalg.LinAlgError:
            # 还是失败：返回NaN
            return np.nan
...
def get_distance(z_m0, z_S0, z_m1, z_S1, by = "KL"):
    ...
    # 处理NaN值：替换为0（表示没有显著变化）
    dis_array = np.array(dis)
    nan_count = np.isnan(dis_array).sum()
    if nan_count > 0:
        print(f"    WARNING: Found {nan_count} NaN distances (SVD failed), replacing with 0...")
        dis_array = np.nan_to_num(dis_array, nan=0.0)
