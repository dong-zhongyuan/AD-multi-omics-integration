...
read_fun_keys = [
    ...
    "mtx",
    "text",
]  # fun from scanpy read


def _read_counts(counts_path: str, transpose: bool = False, **kwargs):
    ...
    if Path(counts_path).is_file() and file_attr in file_attrs:
        ...
        if file_attr == "mat":
            ...
            if transpose:
                adata = read_fun(
                    counts_path, **kwargs
                ).transpose()  # transpose counts file
            else:
...
def build_adata(
    ...
    **kwargs,
    ...
    if uppercase:
        ...
        )  # all species use upper case genes
...
def split_data(dir: str = "data", data=None, load: bool = False, save: bool = False):
    ...
    if load:
        train_data, val_data, test_data = (
            load_gdata(path, "train_data"),
            load_gdata(path, "val_data"),
            load_gdata(path, "test_data"),
        )
    ...
    if num_edges < 50:
        # 边数太少，使用更小的验证集和测试集比例
        num_val = max(1, int(num_edges * 0.1))  # 至少1条边
        num_test = max(1, int(num_edges * 0.2))  # 至少1条边
    ...
    # 为 val_data 添加负样本（如果没有或为空）
    if not hasattr(val_data, "neg_edge_label_index"):
        from torch_geometric.utils import negative_sampling

        num_neg = (
            val_data.pos_edge_label_index.size(1)
            if hasattr(val_data, "pos_edge_label_index")
            else 1
        )
        if num_neg > 0:
            val_data.neg_edge_label_index = negative_sampling(
    ...
    # 删除不需要的标签，但保留 neg_edge_label_index
    if hasattr(train_data, "pos_edge_label"):
        del train_data.pos_edge_label
    if hasattr(train_data, "neg_edge_label"):
        del train_data.neg_edge_label
    return train_data, val_data, test_data
