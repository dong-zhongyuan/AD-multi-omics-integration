...
class VGAE_trainer:
    ...
    def _transform_data(
        ...
        **kwargs,
        ...
        if x_dropout is not None:
            mask = (
                torch.FloatTensor(self.data.x.shape).uniform_() > x_dropout
            )  # % zeros
        ...
        if edge_noise is not None:
            ...
            if edge_noise > 0:  # fold edges
                ...
                fake_pos_edge = negative_sampling(
                    self.data.edge_index,  # then fake edges impossible appeared in test set: for data leakage
                    num_neg_samples=n,
                    num_nodes=len(self.train_data.x),
                )
                new_pos_edge = torch.unique(
                    torch.cat((self.train_data.pos_edge_label_index, fake_pos_edge), 1),
                    dim=1,
                )
                new_edge = torch.cat(
                    (new_pos_edge, new_pos_edge[torch.LongTensor([1, 0])]), 1
                )  # swap
                print(
                    f"add noise to training data edge, level: {edge_noise}: add {n} edges"
                )
            else:  # ratio edges
                if n > n_pos_edge:
                    raise ValueError("cannot retain edges more than the total")
                weights = torch.tensor(
                    [1 / n_pos_edge] * n_pos_edge, dtype=torch.float
                )  # uniform weights
                ...
                print(
                    f"retain a portion of training data edge, level: {abs(edge_noise)}: retain {n} edges"
                )
            self.train_data.edge_index, self.train_data.pos_edge_label_index = (
                new_edge,
                new_pos_edge,
            )
    ...
    def train(self, **kwargs):
        ...
        self.train_data, self.val_data, self.test_data = (
            ...
            self.val_data.to(device),
            self.test_data.to(device),
        )
        ...
        for epoch in range(self.epochs):
            ...
            recon_loss = self.model.recon_loss(
                z,
                self.train_data.pos_edge_label_index,
                self.train_data.neg_edge_label_index,
            )
            ...
            if self.logging:
                ...
                self.train_logger.add_scalar(
                    "recon_loss", recon_loss.item(), global_step
                )
            ...
            with torch.no_grad():
                ...
                # 确保 test_data 有 neg_edge_label_index
                if not hasattr(self.test_data, "neg_edge_label_index"):
                    from torch_geometric.utils import negative_sampling

                    self.test_data.neg_edge_label_index = negative_sampling(
                        edge_index=self.test_data.edge_index,
                        num_nodes=self.test_data.num_nodes,
                        num_neg_samples=self.test_data.pos_edge_label_index.size(1),
                    ).long()
                else:
                    # 确保数据类型正确
                    self.test_data.neg_edge_label_index = (
                        self.test_data.neg_edge_label_index.long()
                    )
                ...
                if self.verbose:
                    print(
                        f"Epoch: {epoch:03d}, Loss: {loss:.4f}, AUROC: {auc:.4f}, AP: {ap:.4f}"
                    )
    ...
    def load_model(self, name: str):
        ...
        r.load_state_dict(
            torch.load(
                os.path.join("model", f"{name}.th"), map_location=torch.device("cpu")
            )
        )
        self.model = r
...
def eva(args):
    ...
    sensei.train(
        edge_noise=args.e_noise,
        x_noise=args.x_noise,
        x_dropout=args.x_dropout,
        dir=args.dir,
        load=False,  # load split data
    )
